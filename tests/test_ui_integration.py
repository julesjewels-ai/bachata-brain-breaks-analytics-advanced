...
def test_app_integration_with_ui():
    ...
    # Run the app (mocking ingestion/processing implicitly by the
    # app's design which mocks data internally)
    asyncio.run(app.run())
    ...
    assert any(
        c[0] == 'success' and "Data loaded" in c[1]
        for c in ui.calls
    )
    assert any(
    ...
    assert any(
        c[0] == 'success' and "Report saved" in c[1]
        for c in ui.calls
    )
    assert any(
