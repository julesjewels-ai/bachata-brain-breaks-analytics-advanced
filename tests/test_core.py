...
class MockDataIngestionService(DataIngestionService):
    def ingest_data(self):
        # Return a simple DataFrame for testing logic not dependent on
        # data values
        return pd.DataFrame({
            'video_id': ['1'],
            'title': ['Test'],
...
def test_ingest_data_structure():
    ai_service = MockAIService()
    report_generator = MockReportGenerator()
    # Use real simulation service to test structure integration
...
def test_outlier_detection():
    ...
    # With 3 items, 90th percentile is high. 'Viral' (10000) should be
    # caught or border case depending on interpolation.
