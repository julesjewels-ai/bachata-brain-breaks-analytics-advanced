...
async def test_ingest_data(mock_youtube_client):
    service = YouTubeIngestionService(
        youtube_client=mock_youtube_client, target_channel_id="UC123"
    )
...
async def test_ingest_data_validation_drop(mock_youtube_client):
    ...
    mock_youtube_client.get_channel_videos = AsyncMock(
        return_value=[
            {"video_id": "VID_1", "title": "Good Video", "views": 100},  # Good
            {
                "video_id": "VID_2",
                "title": "Bad Video",
                "views": -50,
