...
class MockVisualizer:
    def generate_chart(
        ...
        # Return a minimal valid PNG byte signature
        return BytesIO(
...
def test_excel_generation_conditional_formatting(tmp_path):
    ...
    assert data_bar_rules >= 2, f"Expected >= 2 rules, found {data_bar_rules}"
...
def test_estimate_cell_width():
    ...
    assert ExcelStyler.estimate_cell_width(
        MockCell(1234.56, '#,##0.00')
    ) == 8
