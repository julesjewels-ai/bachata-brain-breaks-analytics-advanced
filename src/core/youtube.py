...
class YouTubeAPIClient:
    ...
    async def get_channel_videos(
        ...
        videos = await self._get_playlist_items(
            uploads_playlist_id, max_results
        )
        ...
        video_stats = await self.get_video_statistics(video_ids)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    ...
                    logger.error(f"YouTube API Error ({
                        response.status}) on /channels: {error_text}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"YouTube API Error ({
                        response.status}) on /playlistItems: {error_text}")
                response.raise_for_status()
                data = await response.json()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"YouTube API Error ({
                        response.status}) on /videos: {error_text}")
                response.raise_for_status()
                data = await response.json()
