...
class ChartBuilder:
    ...
    def add_bar_chart(self,
        ...
        Args:
            ...
            anchor: The cell where the top-left corner of the chart
                    will be placed.
        """
        chart = BarChart()
        ...
        # Categories (Labels)
        # Use explicit location if provided, otherwise default to column
        # left of data
        cats_min_col = (
            data_loc.cats_min_col if data_loc.cats_min_col is not None
            else data_loc.min_col - 1
        )

        # Default rows usually match data rows (adjusted for header if needed)
        default_min_row = (
            data_loc.min_row + 1 if data_loc.title_from_data
            else data_loc.min_row
        )
        cats_min_row = (
            data_loc.cats_min_row if data_loc.cats_min_row is not None
            else default_min_row
        )

        cats_max_row = (
            data_loc.cats_max_row if data_loc.cats_max_row is not None
            else data_loc.max_row
        )
