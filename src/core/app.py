...
class BachataAnalyticsApp:
    ...
    async def _run_gemini_analysis(self, df: pd.DataFrame) -> Optional[str]:
        ...
        from src.core.repository import RepositoryError
        # Archive the selected inputs before generating analysis
        try:
            ...
            self.notification_service.notify(NotificationEvent(
                ...
                message=f"Could not archive data, but analysis will continue. Details: {e}",
                level='warning'
            ))
