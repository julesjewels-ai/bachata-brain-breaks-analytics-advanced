...
class BachataAnalyticsApp:
    ...
    def _display_anomalies(self, anomalies: Dict[str, pd.DataFrame]) -> None:
        """Displays detected anomalies in the UI."""
    ...
    async def _stream_and_capture(
        self, stream: AsyncGenerator[str, None], storage: List[str]
    ) -> AsyncGenerator[str, None]:
        """Helper to capture stream chunks while yielding them."""
        async for chunk in stream:
            storage.append(chunk)
            yield chunk

    async def _run_gemini_analysis(self, df: pd.DataFrame) -> Optional[str]:
        """Runs the Gemini analysis and streams output to UI."""
        ...
        await self.ui.display_stream(self._stream_and_capture(stream, strategy_chunks))
        return "".join(strategy_chunks)

    def _generate_report_safe(self, anomalies: Dict[str, pd.DataFrame], strategy: str) -> None:
        """Generates the Excel report with error handling."""
        try:
    ...
    async def run(self) -> None:
        ...
        self._generate_report_safe(anomalies, strategy)
