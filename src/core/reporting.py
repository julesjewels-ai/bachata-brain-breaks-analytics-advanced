...
class ExcelReportGenerator:
    ...
    def _process_anomaly_sheets(self, writer: pd.ExcelWriter, anomalies: Dict[str, pd.DataFrame]):
        """Processes anomaly dataframes and creates styled sheets with charts."""
    ...
    def _add_anomaly_chart(self, ws: Worksheet, v_type: str):
        """Adds a bar chart for views to the worksheet."""
        # Locate 'Views' column
        headers = ExcelReportGenerator._get_header_map(ws)
    ...
    def _add_strategy_sheet(self, writer: pd.ExcelWriter, strategy: str):
        """Adds a sheet with the strategy analysis text."""
    ...
    def _add_visual_insights(self, writer: pd.ExcelWriter, anomalies: Dict[str, pd.DataFrame]):
        """Adds a sheet with visual insights (chart)."""
