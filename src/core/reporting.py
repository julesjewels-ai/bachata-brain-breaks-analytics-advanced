...
class ExcelReportGenerator:
    ...
    def _create_anomaly_sheet(self, writer, v_type: str, df: pd.DataFrame):
        """Creates and styles a single anomaly sheet with charts."""
    ...
    def _create_strategy_sheet(self, writer, strategy: str):
        """Creates the strategy sheet."""
        ...
        ws_strat['A2'].alignment = Alignment(wrap_text=True,
                                             horizontal='left',
                                             vertical='top')

    def _create_visual_insights_sheet(self, writer,
                                      anomalies: Dict[str, pd.DataFrame]):
        """Creates the visual insights sheet with a scatter plot."""
        # Combine all anomalies to one DF for visualization
        all_anomalies = (pd.concat(anomalies.values()) if anomalies
                         else pd.DataFrame())
        if (not all_anomalies.empty and
            ...
            try:
                ...
                # Add description
                ws_viz["A25"] = ("Scatter plot showing relationship between "
    ...
    def generate_excel(self,
        ...
        with pd.ExcelWriter(safe_path, engine='openpyxl') as writer:
            ...
            for v_type, df in anomalies.items():
                if not df.empty:
                    self._create_anomaly_sheet(writer, v_type, df)

            # 2. Strategy Sheet
            self._create_strategy_sheet(writer, strategy)

            # 3. Visual Insights (Embedded Matplotlib)
            self._create_visual_insights_sheet(writer, anomalies)
