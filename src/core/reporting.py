...
class ExcelReportGenerator:
    ...
    def _create_anomaly_sheet(self, writer: pd.ExcelWriter, v_type: str, df: pd.DataFrame):
    ...
    def _add_anomaly_chart(self, ws: Worksheet, v_type: str):
        """Adds a bar chart to the anomaly sheet if valid columns exist."""
        # Locate 'Views' column
        headers = ExcelReportGenerator._get_header_map(ws)
    ...
    def _add_visual_insights(self, writer: pd.ExcelWriter, anomalies: Dict[str, pd.DataFrame]):
        """Generates and embeds the Matplotlib visualization."""
