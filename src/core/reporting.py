...
class ExcelReportGenerator:
    ...
    def _create_anomalies_sheet(self, writer: pd.ExcelWriter, v_type: str, df: pd.DataFrame):
        """Creates and formats a sheet for viral anomalies."""
        if df.empty:
            return
        ...
        # Add Chart
        self._add_views_chart(ws, v_type)

    def _add_views_chart(self, ws, v_type: str):
    ...
    def _create_strategy_sheet(self, writer: pd.ExcelWriter, strategy: str):
        """Creates and formats the strategy analysis sheet."""
    ...
    def generate_excel(self,
        ...
        with pd.ExcelWriter(safe_path, engine='openpyxl') as writer:
            ...
            for v_type, df in anomalies.items():
                self._create_anomalies_sheet(writer, v_type, df)
