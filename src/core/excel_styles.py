...
Handles styling and formatting logic for Excel worksheets.
"""
...
class ExcelStyler:
    ...
    @staticmethod
    def estimate_cell_width(cell: Any) -> int:
    ...
    @staticmethod
    def adjust_column_widths(ws: Worksheet, min_width: int = 10, max_width: int = 50) -> None:
        ...
        for col in ws.columns:
            ...
            # col[0] is the header, assume we want to size for it too
    ...
    @staticmethod
    def apply_number_formats(ws: Worksheet, format_map: Dict[str, str]) -> None:
    ...
    @staticmethod
    def apply_conditional_formatting(ws: Worksheet, rules: Dict[str, DataBarRule]) -> None:
        """Applies data bars (or other rules) to visualization columns."""
        headers = ExcelStyler.get_header_map(ws)
