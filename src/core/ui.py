...
class RichConsoleUI:
    ...
    def display_table(
        ...
        for col_name in data.columns:
            ...
            is_numeric = pd.api.types.is_numeric_dtype(data[col_name])
            is_metric = col_name in ["Views", "Retention", "Retention (%)"]

            if is_numeric or is_metric:
                justify = "right"
                style = "green"
        ...
        for _, row in data.iterrows():
            ...
            for val in row:
                ...
                # or huge number?
                # Actually, let's rely on the caller to format values if
                # specific formatting (like %) is needed.
