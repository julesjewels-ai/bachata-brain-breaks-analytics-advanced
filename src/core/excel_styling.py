...
Handles all styling and formatting logic for Excel reports (SRP).
"""
...
class ExcelStyler:
    """Handles styling and formatting logic for Excel sheets."""
    ...
    def estimate_cell_width(cell) -> int:
        ...
        # Check for bold font
        is_bold = False
        if hasattr(cell, 'font') and cell.font and cell.font.b:
            is_bold = True
    ...
    @staticmethod
    def apply_table_borders(ws: Worksheet):
        """Applies thin borders to the entire data table (including headers)."""
        if ws.max_row < 1 or ws.max_column < 1:
            return

        for row in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
            for cell in row:
                cell.border = ExcelStyler.BORDER_SET
