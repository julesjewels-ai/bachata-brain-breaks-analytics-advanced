...
## 2025-02-18 - [Complex Method] **Observation:** `ExcelReportGenerator.generate_excel` in `src/core/reporting.py` was handling sheet loop, styling, and multiple chart types inline, violating SRP. **Action:** Extracted `_process_anomaly_sheets`, `_add_strategy_sheet`, `_add_visual_insights`, and `_add_anomaly_chart` helper methods.
